...
public interface ContactRepository extends JpaRepository<ContactEntity, Long> {
    ...
    @Query(value = "SELECT c.* FROM MEMBERSCONTACTS c " +
            "JOIN MEMBERCONTACTMAP m ON c.CONTACTID = m.CONTACTID " +
            "WHERE m.MEMBERID = :memberId", nativeQuery = true)
    List<ContactEntity> findByMemberId(@Param("memberId") String memberId);

    @Modifying
    @Query(value = "INSERT INTO MEMBERCONTACTMAP (MEMBERID, CONTACTID) " +
            "VALUES (:memberId, :contactId)", nativeQuery = true)
